...
func _log_device_info() -> void:
    event( \
            ...
            int(Utils.get_viewport_diagonal_inches() * 1000), \
            true)
...
func _get_payload( \
    ...
    return ( \
        ...
        viewport_size_str,
        OS.get_locale(),
        Constants.APP_NAME.http_escape(),
        Constants.APP_ID.http_escape(),
        Constants.APP_VERSION.http_escape(),
        str(randi()),
    ]
